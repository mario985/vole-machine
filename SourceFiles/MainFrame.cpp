...
void MainFrame::ImportButtonClicked(wxCommandEvent& evt){
	...
	if (dialog.ShowModal() == wxID_OK) { 
		...
		if (file.is_open()) {
			...
			if (checkMemoryaddress(string(dialog.GetAdd()))) 
				Program.getCpu().getPc() = Program.getCpu().getALU().HexToDec(string(dialog.GetAdd()));		

			else  Program.getCpu().getPc() = Program.getCpu().getALU().HexToDec(("0A")); 
		}
		else {
	...
	if (checkMemoryaddress(start)) {
		Program.getCpu().Import("Text.txt", Program.getCpu().getALU().HexToDec(start));
	}
	else {
		Program.getCpu().Import("Text.txt", Program.getCpu().getALU().HexToDec("0A"));
	}
...
bool MainFrame::checkMemoryaddress(string Address){
	bool check = 0;
	regex pattern(R"([0-9A-F]{1}[0-9A-F]{1})");
